sap.ui.define(
  ...
  (
    ...
    const I18N_KEYS = {
      ...
      CLIPBOARD_COPIED_SUCCESS: "clipboard.copied.success",
      CLIPBOARD_COPIED_FAILURE: "clipboard.copied.failure",
    };
    ...
    return Controller.extend("com.company.demo.controller.BaseController", {
      ...
      showError(vError, sTitle) {
        const sMessage =
          typeof vError === "string"
            ? vError
            : vError.message || this.getText(I18N_KEYS.ERROR_UNKNOWN);
        ...
        try {
          ...
          if (oError.responseText?.trim().startsWith("{")) {
            ...
            if (oErrorData?.innererror?.errordetails) {
              this._showDetailedServiceError(
                oErrorData.innererror.errordetails
              );
              return;
            }
            sErrorMessage =
              oErrorData?.message?.value ||
              oErrorData?.message ||
              sErrorMessage;
          }
          // 2. XML Response
          else if (
            oError.responseText?.includes("<?xml") ||
            oError.responseText?.includes("<error>")
          ) {
            sErrorMessage =
              this._parseXmlError(oError.responseText) || sErrorMessage;
          }
          // 3. Status Text (HTTP errors)
          else if (
            oError.statusText &&
            oError.statusText !== "HTTP request failed"
          ) {
      ...
      _parseXmlError(sXmlResponse) {
        try {
          ...
          const sMessageMatch = sXmlResponse.match(
            /<message[^>]*>([^<]+)<\/message>/i
          );
          ...
          const sErrorMatch = sXmlResponse.match(
            /<error[^>]*>([\s\S]*?)<\/error>/i
          );
          ...
          if (typeof DOMParser !== "undefined") {
            try {
              ...
              const oMessageElement =
                oXmlDoc.getElementsByTagName("message")[0];
              if (oMessageElement) {
      ...
      callService(sMethod, sPath, oArgs) {
        ...
        if (!oModel || typeof oModel[sMethod] !== "function") {
          return Promise.reject(
            new Error(`OData method '${sMethod}' not found.`)
          );
        }
        ...
        return new Promise((resolve, reject) => {
          ...
          const oParams = {
            ...
            success: (oData) => {
              resolve(
                sMethod === "read" || sMethod == "callFunction"
                  ? oData.results || oData
                  : oData
              );
            },
        ...
        return new Promise((resolve) => {
          ...
          const oSelectDialog = new SelectDialog({
            ...
            search: (oEvent) => {
              ...
              const aFilters = aSearchFields.map(
                (sField) => new Filter(sField, FilterOperator.Contains, sValue)
              );
            ...
            confirm: (oEvent) => {
              ...
              const vResult = bMultiSelect
                ? aSelectedObjects
                : aSelectedObjects[0] || null;
        ...
        return new Promise((resolve) => {
          ...
          const oHtml = new HTML({
            ...
            afterRendering: () => {
              ...
              const draw = (oEvent) => {
                if (!bDrawing) return;
                const { offsetX, offsetY } = getCoords(oEvent);
                oCtx.beginPath();
      ...
      async exportTableToExcel(vTable, sFileName = "Export") {
        const oTable =
          typeof vTable === "string" ? this.getView().byId(vTable) : vTable;
        if (!oTable) return;
        ...
        return new Promise((resolve, reject) => {
          ...
          script.onload = () => {
            if (window[sGlobalVariableName]) {
              ...
              reject(
                new Error(
                  `${sGlobalVariableName} global variable was not available after loading the script.`
                )
              );
            }
          };
       ...
       * @public
       * @async
       * @param {string} sText The text content to be copied to the clipboard.
       * @returns {Promise<void>} A Promise that resolves when the operation is complete. The Promise is rejected upon failure.
       */
      async copyTextToClipboard(sText) {
        const sSuccessMessage = this.getText(
          I18N_KEYS.CLIPBOARD_COPIED_SUCCESS
        );
        const sFailMessage = this.getText(I18N_KEYS.CLIPBOARD_COPIED_FAILURE);
        ...
        try {
          if (navigator.clipboard && navigator.clipboard.writeText) {
            ...
            oTextarea.style.pointerEvents = "none";
          ...
          console.error("Clipboard copy error:", oError);
          MessageToast.show(sFailMessage);
      ...
      _serviceTypeToUI5MessageType(sType) {
        ...
        switch (sNormalizedType) {
          ...
          default:
            return (
              sNormalizedType?.charAt(0).toUpperCase() +
                sNormalizedType?.slice(1) || "Error"
            );
        }
      },
